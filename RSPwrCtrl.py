...
config = Config(RepositoryEnv('InstanciaPwr/settings.ini'))
import schedule
import time
import datetime
import functools
...
TeclaEnter = "\r\n"
LOG_FILE = "InstanciaPwr/telnet_commands.log"
LOG_RESULTS = "InstanciaPwr/results.log"
...
class NBI:
    ...
    def get_network_status(self) -> None:
        ...
        try:
            ...
            df1['Timestamp'] = datetime.datetime.now().strftime('%Y-%m-%d %H:%M')
...
def AjusteDePotencia(sentido, localcellid, cell, RS_MAX):
    """
...
if __name__ == "__main__":
    ...
    RefPwr = pd.read_excel(r'InstanciaPwr/CELULAS_PWR.xlsx')
                ...
                for cell in site_cells:                
                    ...
                    if localcellid is not None:
                        ...
                        if num_users > 1.1 * MaxUsers(cell):
                            AjusteDePotencia('reduzir', localcellid, cell, RS_MAX)
                        elif num_users < 0.9 * MaxUsers(cell):
                            AjusteDePotencia('aumentar', localcellid, cell, RS_MAX)
                        else:
                            pass
...
def parse_pdsch_data(data_string):
    ...
    return [dict(zip(headers, row)) for row in all_data]


#  teste = parse_pdsch_data(texto)
# texto
# teste